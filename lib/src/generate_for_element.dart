...
Future<String> generateForElement<T>(
  ...
  final elements = libraryReader.allElements
      .where((e) => e.name == name)
      .toList();
  ...
  NOTE: Could not find an annotation that matched
    ...
    Using a annotation with the same name from the synthetic library instead
      ${(annotation.type as InterfaceType).element.library.firstFragment.source.uri}#${annotation.type!.element!.name}''';
    } else {
      msg =
          '''
  NOTE: Could not find an annotation that matched
